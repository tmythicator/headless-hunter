...
- *Harvester (Scraping)*: ~bun test tests/integration/glassdoor.test.ts~
- *Reasoning (Ollama)*: ~bun test tests/integration/ollama.test.ts~
- *Visuals*: ~bun run test:render~ (Debug terminal markdown rendering)
