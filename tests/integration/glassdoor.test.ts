...
import { harvestLinksFromPage, scrapeContentLocal } from "../../src/tools";
...
describe("Glassdoor Harvester (Live)", () => {
  ...
  for (const testCase of TEST_CASES) {
    test(`should harvest links from ${testCase.name}`, async () => {
      ...
      if (links.length > 0) {
        ...
        console.warn(`Harvested 0 links from ${testCase.url}. This might be due to anti-bot protection.`);
      }
    }, TIMEOUT);
  }
});
