...
export async function scoutNode(state: typeof AgentState.State) {
    ...
    if (scrapedKnowledge.length < 500) {
       ...
       if (directLinks.length > 0) {
           ...
           scrapedKnowledge += scrapedDocs.map((d: { url: string, raw_content?: string, content?: string }) => `--- TAVILY SOURCE: ${d.url} ---\n${d.raw_content || d.content}`).join("\n\n");
       }
    }
    ...
    **Market Summary:** A 1-sentence honest (cynical) assessment of the market today.
    ...
    // Write result
    try {
        await Bun.write("result.md", analysisText);
    } catch (e) {
