...
export async function profilerNode(state: typeof AgentState.State) {
  const model = getModel(ModelRole.PROFILER);
...
export async function scoutNode(state: typeof AgentState.State) {
  const model = getModel(ModelRole.SCOUT);
  ...
  try {
      ...
      analysisText = "Check logs. Model failed to generate report.";
  }
  ...
  return { messages: [new HumanMessage(analysisText)] };
}